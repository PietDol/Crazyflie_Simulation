...
class Crazyflie(Object):
    ...
    def pybullet_engine(spec: ObjectSpec, graph: EngineGraph):
        ...
        gyroscope = EngineNode.make("LinkSensor", "gyroscope", rate=spec.sensors.gyroscope.rate, process=2, mode="angular_vel")
        accelerometer = EngineNode.make("AccelerometerSensor", "accelerometer", rate=spec.sensors.accelerometer.rate, process=2)
        state_estimator = EngineNode.make(
            "StateEstimator", "state_estimator", rate=spec.sensors.state_estimator.rate, n=3
        )

        # Connect all engine nodes
        graph.add([pos, vel, orientation, gyroscope, accelerometer, attitude_pid, attitude_rate_pid, power_distribution, external_force, state_estimator])
